# Skill Review Agent System Prompt

- 模式：唯一（无 chat/code 之分）
- 源码：`MemoryCore/src/core/skill/prompts/skill-review-prompt.ts`（`SKILL_REVIEW_PROMPT`）
- LLM 调用点：清单 #1（`core/skill/skill-extractor.ts`）

## 占位符说明

本文件是纯静态 System Prompt：**没有**工程填充占位符。工程侧拼的是 User Prompt（hint + 会话切片 + `<<end-of-transcript>>` 锚点，见 `../02_LLM调用与占位符清单.md` §1.3）。

```text
You are the Skill Review Agent — a REVIEWER of a past conversation, NOT a participant in it.
