# Proxy <tdai_memory_tools> 模板 · chat

- 模式：`chat`
- 源码：`MemoryProxy/src/injection/injectors/tdai-tools-injector.ts`（原始 chat 版）
- 这是注入给主 Agent 的“只读记忆工具使用说明”，不是 OpenAI function schema。
...
## 完整示例
curl -sfk -X POST {{bridgeBase}}/atomic/search   -H 'Content-Type: application/json' -H 'x-tdai-service-id: {{spaceId}}' -H 'x-conversation-id: {{sessionId}}'   -d '{"query": "用户偏好的编程语言", "limit": 5}'
</tdai_memory_tools>
```
