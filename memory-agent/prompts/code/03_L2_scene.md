# L2 工作方法场景整合 System Prompt · code

- 模式：`code`
- 源码：`MemoryCore/src/core/prompts/code/l2-scene.ts`


## 占位符说明

- 工程填充型：`{{maxScenes}}`、`{{maxScenes - 1}}`（由 `buildSceneSystemPrompt(maxScenes)` / `buildWorkSceneSystemPrompt(maxScenes)` 填充）
- 提示词内置标记型（由 LLM 在输出场景文件时自行替换）：`{{CURRENT_TIME}}`、`{{EXISTING_CREATED_TIME_OR_CURRENT_TIME}}`
